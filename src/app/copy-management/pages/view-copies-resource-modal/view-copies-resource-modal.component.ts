...
export class ViewCopiesResourceModalComponent implements OnInit{
    ...
    public selectedCopy!:any;
    public resourceCopies!:Copy[];
    @Output() public onCopyUpdate:EventEmitter<Copy>=new EventEmitter<Copy>;
    ...
    updateCopy (copy: Copy){
      this.resourceService.updateCopy(copy).subscribe ({
        next: (resp) => this.onCopyUpdate.emit(copy),
